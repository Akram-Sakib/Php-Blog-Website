...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($mailcheck != false) {
        ...
        $to = "$email";
        $from = "sayedakramsakib@gmail.com";
        $headers = "From: $from\n";
        $headers .= "MIME-Version: 1.0" . "\r\n";
        $headers .= "Content-type: text/html; charset=UTF-8" . "\r\n";
        $subject = "Your Password";
        $message = "Your username is".$username."and password is ".$newpass." Pleas visit website to login";

        $sendmail =  mail($to, $subject, $message, $headers);
        if ($sendmail) {
            echo "<span style='color:green;font-size:18px;'>Please check your email for new passoword!</span>";
        }else {
